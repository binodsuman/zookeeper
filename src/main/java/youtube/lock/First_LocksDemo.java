...
public class First_LocksDemo {
    static ZooKeeper zooKeeper;
    public static void main(String[] args) throws IOException, InterruptedException, KeeperException {
        String sessionId = UUID.randomUUID().toString();
        String lockNode = "/locks";
        Watcher watcher = new Watcher() {
            @Override
            public void process(WatchedEvent watchedEvent) {
                System.out.println("Eent type : "+watchedEvent.getType());
                if(watchedEvent.getType() == Event.EventType.NodeChildrenChanged){
                    try {
                        getLockOrWait(sessionId, lockNode);
        ...
        if(zooKeeper.exists(lockNode, false) ==  null){
            zooKeeper.create(lockNode, new byte[0], ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
        }
        zooKeeper.create(lockNode+ "/"+ "lock-", sessionId.getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL_SEQUENTIAL);
        getLockOrWait(sessionId, lockNode);
    }
    ...
    private static void getLockOrWait(String sessionId, String rootNode) throws KeeperException, InterruptedException {
        ...
        if(data!=null && new String(data).equalsIgnoreCase(sessionId)){
            System.out.println("I acquired a lock  by Server 1:). will leave it in 10 seconds");
            for (int i=0;i<10;i++){
